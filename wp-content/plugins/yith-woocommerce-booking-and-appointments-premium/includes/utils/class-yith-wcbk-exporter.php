					...
					case 'export_future_ics':
						if ( ! empty( $_REQUEST['product_id'] ) ) {
							...
							if ( $product ) {
								...
								if ( ! ! $key && yith_wcbk_is_booking_product( $product ) && $product->is_valid_external_calendars_key( $key ) ) {
			...
			foreach ( $post_ids as $id ) {
				...
				if ( $booking->is_valid() ) {
					...
					$description_data = array(
						'FROM'   => $booking->get_formatted_from(),
						'TO'     => $booking->get_formatted_to(),
						'STATUS' => $booking->get_status_text(),
						'PEOPLE' => $booking->get_persons(),
					);
